...
> A synthesizable 4-bit ALU written in Verilog supporting 8 operations — ADD, SUB, AND, OR, XOR, NOT, LSL, LSR — with carry, overflow, zero, and negative flag outputs, verified with an exhaustive self-checking testbench.
...
- ✅ **Exhaustive testbench** — 256-case sweeps for ADD and SUB plus targeted edge-case checks
...
Synthesized to a generic gate-level netlist — purely combinational, zero flip-flops:

| Metric | Value |
|--------|-------|
| **Flip-Flops** | 0 — fully combinational design |
| **Gate Types** | AND, OR, XOR, NOT |
| **Netlist Formats** | Verilog (`alu_4bit_synth.v`), JSON (`alu_4bit.json`) |
| **Schematic** | PNG via Graphviz (`alu_4bit.png`) |

Run `make synth` to reproduce the netlist and view the full cell statistics from Yosys.
...
```bash
yosys synth.ys    # Run Yosys synthesis script, generate netlist + schematic
```
...
| `make wave` | Open `alu_4bit.vcd` in GTKWave |
| `make clean` | Remove generated `.vvp` and `.vcd` files |

---

## ✅ Testbench Coverage

The testbench (`alu_4bit_tb.v`) runs a two-tier verification suite:

**Targeted tests** — named edge cases with explicit expected values for all 5 outputs:

| Test | Checks |
|------|--------|
| ADD: `5 + 3`, `15 + 1`, `0 + 0`, `7 + 1`, `-8 + -1` | Result, carry, zero, overflow, negative |
| SUB: `8 - 3`, `5 - 5`, `0 - 1` | Result, borrow, zero, overflow, negative |
| Logical: AND, OR, XOR, NOT on `1010` / `1100` | Result and all flags |
| Shifts: LSL and LSR on `1010` | Result and carry-out |

**Exhaustive sweeps** — all 256 input combinations for ADD and SUB:

```
for i in 0..15:
  for j in 0..15:
    verify result[ADD] == (i + j) & 0xF
    verify result[SUB] == (i - j) & 0xF
```
